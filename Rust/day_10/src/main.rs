...
use aoc::{self, Point, get_neighbours_4};
...
fn parse_input(lines: Vec<&str>) -> (HashMap<Point, i32>, Vec<Point>, Point) {
    ...
    for (y, line) in lines.iter().enumerate() {
        for (x, c) in line.chars().enumerate() {
            if c != '.' {
                points.insert(Point { x: x as isize, y: y as isize}, c.to_digit(10).unwrap() as i32);
            }
            if c == '0' {
                starts.push(Point { x: x as isize, y: y as isize });
            }
        }
    }
    ...
    while !cur.is_empty() {
        for &p in &cur {
            for n in get_neighbours_4(p, p_max).into_iter().filter(|n| {
                points.get(n).map_or(false, |&value| value - points[&p] == 1)
            }) {
